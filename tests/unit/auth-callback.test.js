...
describe("GET /api/auth/callback (OAuth security boundary)", () => {
  ...
  it("state param mismatched with the fx_oauth_state cookie redirects to /?auth=error", async () => {
    const res = mockRes();
    await authCallback(req({ code: "c1", state: "state-from-google" }, "fx_oauth_state=different-state"), res);
  ...
  it("valid state but non-owner email redirects to /?auth=denied and never sets fx_session", async () => {
    global.fetch
    ...
    const res = mockRes();
    await authCallback(req({ code: "c1", state: "matching-state" }, "fx_oauth_state=matching-state"), res);

    expect(res.statusCode).toBe(302);
    expect(res.headers.Location).toBe("/?auth=denied");
    expect(setCookieText(res)).not.toMatch(/fx_session=/);
  });

  it("valid state and owner email redirects to / and sets fx_session", async () => {
    global.fetch
      .mockResolvedValueOnce({ json: async () => ({ access_token: "AT" }) })
    ...
    expect(setCookieText(res)).toMatch(/fx_session=/);
  });
});
