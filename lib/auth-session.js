...
export function readCookie(req, name) {
  ...
  if (!m) return null;
  // A malformed percent-encoding must read as 'no cookie' (401), never a 500.
  try { return decodeURIComponent(m[1]); } catch { return null; }
}

// __Host- is a stronger cookie: the browser refuses to set it unless the
// response is Secure, Path=/, and has no Domain attribute (all already true
// here) - it also blocks a subdomain from ever overwriting it. Only usable
// over HTTPS, so plain "fx_session" is kept for local (non-secure) dev.
export function sessionCookieName(secure) {
  return secure ? `__Host-${COOKIE}` : COOKIE;
}
