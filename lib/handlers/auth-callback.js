import { signSession, cookie, readCookie, appOrigin, MAX_AGE, sessionCookieName } from "../auth-session.js";
...
export default async function authCallback(req, res) {
  if (!(await guard(req, res, { limit: { key: "callback", limit: 20, windowMs: 60000 } }))) return;
  ...
  const stateCookie = readCookie(req, "fx_oauth_state");
  const clearState = cookie("fx_oauth_state", "", { maxAge: 0 });
  ...
  try {
    ...
    return redirect(res, "/", [clearState, cookie(sessionCookieName(secure), token, { maxAge: MAX_AGE, secure })]);
  } catch {
    return redirect(res, "/?auth=error", clearState);
  }
}
