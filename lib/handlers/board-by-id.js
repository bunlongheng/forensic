...
export default async function boardById(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const { rows } = await db.query(
      "SELECT id, title, slug, nodes, edges, type, tags, created_at, updated_at FROM boards WHERE id = $1 AND trashed_at IS NULL",
      [id],
    );
  ...
  if (req.method === "PUT") {
    ...
    const { error } = validateBoardFields({ title, nodes, edges, tags }, { partial: true });
