...
// Also reports `version` so the prod monitor can confirm a push actually
// deployed before running its checks.
export default async function health(req, res) {
  const checks = {
    api_secret: Boolean(process.env.FORENSIC_API_SECRET?.trim()),
  ...
  return res.status(ok ? 200 : 503).json({ ok, version: VERSION, checks });
}
